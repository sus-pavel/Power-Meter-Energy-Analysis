import { Activity, AlertTriangle, CheckCircle2, Clock, Gauge, HelpCircle, Server, WifiOff } from "lucide-react";
...
export function OperationalStatusCards({ status }: { status: OperationsStatus }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-9">
      ...
      <StatusCard label="Polling Running" value={status.polling_running ? "Yes" : "No"} icon={<Activity className="h-5 w-5" />} />
      <StatusCard label="Measurements / Hour" value={status.measurements_last_hour} icon={<Clock className="h-5 w-5" />} />
    </div>
  );
}
